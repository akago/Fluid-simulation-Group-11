...
RigidBody::RigidBody(Vec2f ConstructPos) : m_ConstructPos(ConstructPos)
{
...
void RigidBody::draw() {
	...
	glBegin(GL_POLYGON);
	for (auto vertex : m_Vertices) {
		glVertex2f(vertex->m_Position[0] + m_Position[0], vertex->m_Position[1] + m_Position[1]);
	}
	glEnd();
}
...
std::vector<float> RigidBody::getBBox() {
	...
	for (auto vertex : m_Vertices) {
		if (minmax[0] > vertex->m_Position[0] + m_Position[0]) minmax[0] = vertex->m_Position[0] + m_Position[0]; 
		if (minmax[1] > vertex->m_Position[1] + m_Position[1]) minmax[1] = vertex->m_Position[1] + m_Position[1]; 
		if (minmax[2] < vertex->m_Position[0] + m_Position[0]) minmax[2] = vertex->m_Position[0] + m_Position[0]; 
		if (minmax[3] < vertex->m_Position[1] + m_Position[1]) minmax[3] = vertex->m_Position[1] + m_Position[1]; 
	}
...
void RigidBody::addTorque() {
	for (auto vertex : m_Vertices) {
		m_torque += crossProduct(vertex->m_Position, vertex->m_Force);
	}
}

void RigidBody::updateR(float angle) {
	m_R.row1[0] = cos(angle);
	m_R.row1[1] = -sin(angle);
	m_R.row2[0] = sin(angle);
	m_R.row2[1] = cos(angle);
}
...
bool RigidBody::verticesInCell(const Vec2f& cell_center, const float h) {
	...
	for (auto vertex : m_Vertices) {
		Vec2f point = vertex->m_Position + m_Position;
...
bool RigidBody::pointInPolygon(const Vec2f& point, const float h) {
	...
	for (int i = 0; i < m_Vertices.size(); i++) {
		Vec2f v1 = m_Vertices[i]->m_Position + m_Position;
		Vec2f v2 = m_Vertices[(i + 1) % m_Vertices.size()]->m_Position + m_Position;
	...
	for (int cell_x = bbox[0]; cell_x < bbox[2]; cell_x++) { 
		...
		for (int cell_y = bbox[1]; cell_y < bbox[3]; cell_y++) { 
			...
			if (cellVerticesInPolygon(cell_center, h) || verticesInCell(cell_center, h)) {
				...
				bnd_vel_u[IX(cell_x, cell_y)] = vel[0];
				bnd_vel_v[IX(cell_x, cell_y)] = vel[1];
